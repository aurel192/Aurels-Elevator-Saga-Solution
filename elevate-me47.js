    ...
    init: function(elevators, floors) {
        _.each(elevators, function(elevator,index) {
            ...
            elevator.on("passing_floor", function(passFloorNum, direction) {
                passingFloor(elevator, passFloorNum, 7, direction);
            });
        ...
        _.each(floors, function(floor) {
            floor.on("down_button_pressed", function() {
                buttonPressedOnFloor( floor.floorNum(), "down_button_pressed", 4, 8, 99999, false);
            });
            floor.on("up_button_pressed", function() {
                buttonPressedOnFloor( floor.floorNum(), "up_button_pressed", 4, 8, 99999, false);
            });
        });
        ...
        addWaitingFloor = function (calledFromFloor, UpOrDownBtn) {  // THIS FUNCTION IS USED TO ALTER ADDITIONAL PROPERTIES OF FLOORS
            ...
            if (UpOrDownBtn == "down_button_pressed"){               // TIME PASSED SINCE THE BUTTON WAS PRESSED.
                floors[calledFromFloor].waitingDn = timer;           // CALCUALTED:  TIMER - FLOORS[5].WAITINGUP
        ...
        sendLeastLoadedElevatorToFloor  = function (floor, UpOrDownBtn, minFreeSpace, minFreeSpace2, sendImmediately) { // THIS FUNCTION IS USED TO SEND THE LEAST LOADED ELEVATOR TO A FLOOR
            ...
            if (LeastLoadedElevator != -1 && LeastLoadedElevatorsFreeSpace>=minFreeSpace) {                    // IF THERE IS AN ELEVATOR WITH ENOUGH SPACE THAT SOULD BE SENT TO THE FLOOR
                if (sendImmediately || ( (LeastLoadedElevatorsFreeSpace >= minFreeSpace2) && minFreeSpace2) ){ // WHEN THIS FUNCTION IS CALLED WITH SENDIMMEDIATELY=TRUE PARAMETER, OR  ALMOST EMPTY
        ...
        buttonPressedOnFloor  = function (floor, UpOrDownBtn, minFreeSpace, minFreeSpace2, maxWaitTime, sendImmediately) {  // UP/DONW BUTTON PRESSED ON A FLOOR
            ...
            ((timer-floors[floor].waitingDn)>maxWaitTime) )
                sendLeastLoadedElevatorToFloor(floor, UpOrDownBtn, minFreeSpace, minFreeSpace2, true);       // THE LEAST LOADED ELEVATOR IS GOING TO PICK THE PASSENGERS UP IMMEDIATELY (NOT EFFICIENT)
            else{                                                                                            //       ELSE (IF IT IS NOT THAT URGENT),
                sendLeastLoadedElevatorToFloor(floor, UpOrDownBtn, minFreeSpace, minFreeSpace2, false);      // THE LEAST LOADED ELEVATOR IS GOING TO PICK THE PASSENGERS UP. (FREE SPACE IS NOT ENSURED ON ARRIVAL)
            }
