    ...
    init: function(elevators, floors) {
        ...
        var listPassangersInElevator = function(logparam) {
       		for (var c = (counter)-20 ; (counter) > 25 && c <= (counter) ; c++ ) {
        		if (passingersInElevators[c][0]>0)
					console.log("PassengersInElevator[" + c + "][0]=" + passingersInElevators[c][0] );
        	}
        }
            ...
            for (var i=NumberOfFloors-1 ; i>=0 ; i--) {
                if (logparam == "ShowLog" ) {
                	if (i==NumberOfFloors-1)
		                console.log("Waiting on Floor " + i + ".            DOWN: " + (time-waitingFloorsDn[i]).toFixed() );                
		            if (i==0)
		            	console.log("Waiting on Floor " + i + ". \tUP: " + (time-waitingFloorsUp[i]).toFixed());                
		            else
                    	console.log("Waiting on Floor " + i + ". \tUP: " + (time-waitingFloorsUp[i]).toFixed() + "\tDOWN: " + (time-waitingFloorsDn[i]).toFixed() );                                
                }
            }
            if (logparam == "ShowLog" )
                console.log("longestWaitingFloorUp " + longestWaitingFloorUp + ": " + (time-waitingFloorsUp[longestWaitingFloorUp]).toFixed() + "s \t longestWaitingFloorDn:" + longestWaitingFloorDn + ": " + (time-waitingFloorsDn[longestWaitingFloorDn]).toFixed() + "s");
        }
