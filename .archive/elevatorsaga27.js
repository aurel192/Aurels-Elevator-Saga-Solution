{
    init: function(elevators, floors) {
       ...
       var rearrangeDestinationQueue = function (elevatorNumber, OriginalDestQ, PressedFloors, currentFloor, logparam) {     
        ...
        var updatePressedButtonsList = function (RemoveThisFloor, msg, logparam) {  // waitingFloorsUp / Dn jobb. lehet hogy ez nemis kell        
        ...
        var floorButtonPressedBehavior = function (elevator, elevatorNumber, FloorBtnPressed, behavior, logparam) {            
            ...
            listPassangersWeightInElevator(elevatorNumber, "FloorButtonPressed",  "NShowLog");
        }
        ...
        var stoppedAtFloorBehavior  = function (elevator, elevatorNumber, Floor, logparam) {
            ...
            if (waitingInsideElevators[elevatorNumber][Floor] != -1) {
                if ("ShowLog")
                    console.log("Passangers in elevator " + elevatorNumber + " delivered to floor " + Floor + " in " + (timer-waitingInsideElevators[elevatorNumber][Floor]).toFixed(1) + "s    loadFactor:" + elevator.loadFactor().toFixed(2) + " maxWaitingInElevator:" + maxWaitingInElevator.toFixed() + " maxWaitingForElevator:" +maxWaitingForElevator.toFixed());
            }
            ...
            listPassangersWeightInElevator(elevatorNumber, "Elevator Stopped", "NOShowLog");           
        }
