{
    init: function(elevators, floors) {
        ...
        // Az utolsó 5-20 értékből a legkisebbet és a legnagyobbat kell kiválasztani, mert egyszerre sokan nyomják meg a gombot. Ebből kideríthető hogy ki, vagy be szálltak többen // de azt a hívó függvénynek kell kiválogatni az utolsó értékekböl, annyiból ahány emelet van.   return currLoad-prevLoad;//console.log("avgWaitTime:" + avgWaitTime + "  maxWaitTime:" + maxWaitTime + " moveCount:" + moveCount + " transportedCounter:" + transportedCounter);
        ...
        visitNearestFirst = function (elevatorNumber, visitFloor, logparam) {        
            ...
            elevators[elevatorNumber].checkDestinationQueue();
            if (logparam == "ShowLog")
                console.log("NearestFloor:" + nearest + " visitFloor:" + visitFloor + " dq:" + elevators[elevatorNumber].destinationQueue);            
        }   
        ...
        passingFloor = function (elevator, passFloorNum, minFreeSpace, direction, logparam) {         
            for (var f=0 ; f<floors.length ; f++) {             
                if ((waitingFloorsUp[f]  ||  waitingFloorsDn[f]) && (  ((1-elevator.loadFactor())*elevator.maxPassengerCount())  >= minFreeSpace  )   ) {
        ...
        stoppedAtFloor  = function (elevator, Floor, logparam) {
            ...
            if (maxWaitingInElevator < timer-waitingInsideElevators[elevator.index][Floor] && waitingInsideElevators[elevator.index][Floor]!=-1)
                maxWaitingInElevator = timer-waitingInsideElevators[elevator.index][Floor];
            ...
            if (waitingInsideElevators[elevator.index][Floor] != -1) {
                if (logparam=="ShowLog")  // Itt akkor is kiirja ha nem bentröl nyomták meg hanem oda hivták a liftet, ezt külön kell választani
                    console.log("Passengers in elevator " + elevator.index + " delivered to floor " + Floor + " in " + (timer-waitingInsideElevators[elevator.index][Floor]).toFixed() + "s    loadFactor:" + elevator.loadFactor().toFixed(2) + " maxWaitingInElevator:" + maxWaitingInElevator.toFixed() + " maxWaitingForElevator:" +maxWaitingForElevator.toFixed());
            }
            waitingInsideElevators[elevator.index][Floor] = -1;
            ...
            if (counter++ > arraySize-50)
                counter=0;
            listPassengersWeightInElevator(elevator.index, "Elevator Stopped", "NoLog");           
        }
        ...
        ButtonPressedOnFloor  = function (floor, UpOrDownBtn, minFreeSpace, sendImmediately, logparam) {       
            SendLeastLoadedElevatorToFloor(floor, UpOrDownBtn, minFreeSpace, sendImmediately, logparam); 
        ...
        _.each(elevators, function(elevator,index) {
            ...
            elevator.on("floor_button_pressed", function(floorNum) {
                floorButtonPressed(elevator, floorNum, "visitNearestFirst", "NoLog");
            });
            elevator.on("passing_floor", function(passFloorNum, direction) {
                passingFloor(elevator, passFloorNum, 4, direction, "ShowLog");   
            });
        ...
        _.each(floors, function(floor) {
            floor.on("down_button_pressed", function() {
                ButtonPressedOnFloor( floor.floorNum(), "down_button_pressed", 3, false, "ShowLog" );
            });             
            floor.on("up_button_pressed", function() {
                ButtonPressedOnFloor( floor.floorNum(), "up_button_pressed", 3, false, "ShowLog" );
            });             
        });     
    },
