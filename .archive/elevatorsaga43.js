
{
    init: function(elevators, floors) {
        ...
        visitFloorWithPriority = function (elevator, floor) {
        ...
        visitFloor = function (elevator, floor) {
        ...
        SendLeastLoadedElevatorToFloor  = function (floor, UpOrDownBtn, minFreeSpace, sendImmediately, logparam) {    // Ha több olyan lift is van amibe férnek még akkor azt kellene küldeni ami közelebb van, és/vagy arra megy
            ...
            if (LeastLoadedElevator != -1 && LeastLoadedElevatorsFreeSpace>=minFreeSpace) {
                if (sendImmediately)                                       
                    visitFloorWithPriority(elevators[LeastLoadedElevator], floor)                                    
                else  
                    visitFloor(elevators[LeastLoadedElevator], floor);
            }      
            else  
                LeastLoadedElevator = -1;                        
            if (UpOrDownBtn == "up_button_pressed") {            
                if (logparam=="ShowLog" && LeastLoadedElevator != -1)        
                    console.log("SendLeastLoadedElevatorToFloor. Up Button Pressed on Floor " + floor + " Dist:" + Dist + " "  + " elevators[" + LeastLoadedElevator + "].destinationQueue:" + elevators[LeastLoadedElevator].destinationQueue + "\nFreeSpace:" + LeastLoadedElevatorsFreeSpace.toFixed(2) + " urgent:" + sendImmediately);            }
            if (UpOrDownBtn == "down_button_pressed") {            
                if (logparam=="ShowLog" && LeastLoadedElevator != -1)        
                    console.log("SendLeastLoadedElevatorToFloor. Down Button Pressed on Floor " + floor + " Dist:" + Dist + " "  + " elevators[" + LeastLoadedElevator + "].destinationQueue:" + elevators[LeastLoadedElevator].destinationQueue + "\nFreeSpace:" + LeastLoadedElevatorsFreeSpace.toFixed(2) + " urgent:" + sendImmediately);
            } 
        }    

        SendNearestElevatorToFloor = function (floor, UpOrDownBtn, minFreeSpace, sendImmediately, logparam) {    // Ha több olyan lift is van amibe férnek még, akkor azt kellene küldeni ami közelebb van, és/vagy arra megy
            var Dist = 999;           
            var nearestElevator = -1;
            for (var e = 0 ; e < elevators.length ; e++ ) {
                if (  (minFreeSpace < ((1 - elevators[e].loadFactor() ) * elevators[e].maxPassengerCount()) )  &&  ( Dist > (Math.abs( elevators[e].currentFloor() - floor ))   )   ) {
                    Dist = Math.abs( elevators[e].currentFloor() - floor );
                    nearestElevator = e;
                }
            }
            if (nearestElevator != -1) {
                if (sendImmediately) 
                    visitFloorWithPriority(elevators[nearestElevator], floor)                                          
                else  
                    visitFloor(elevators[nearestElevator], floor);                
            }   
            if (UpOrDownBtn == "up_button_pressed" && logparam=="ShowLog" && nearestElevator != -1)           
                console.log("SendNearestElevatorToFloor. Up Button Pressed on Floor " + floor + " Dist:" + Dist + " "  + " elevators[" + nearestElevator + "].destinationQueue:" + elevators[nearestElevator].destinationQueue + " urgent:" + sendImmediately);         
            if (UpOrDownBtn == "down_button_pressed" && logparam=="ShowLog" && nearestElevator != -1)                           
                console.log("SendNearestElevatorToFloor. Down Button Pressed on Floor " + floor + " Dist:" + Dist + " "  + " elevators[" + nearestElevator + "].destinationQueue:" + elevators[nearestElevator].destinationQueue + " urgent:" + sendImmediately);            
        }    

        ButtonPressedOnFloor  = function (floor, UpOrDownBtn, minFreeSpace, maxWaitTime, sendImmediately, logparam) {   
            if ( sendImmediately || ((timer-floors[floor].waitingUp)>maxWaitTime) || ((timer-floors[floor].waitingDn)>maxWaitTime) )     
                SendLeastLoadedElevatorToFloor(floor, UpOrDownBtn, minFreeSpace, true, logparam); 
            else
        ...
        _.each(elevators, function(elevator,index) {
            ...
            elevator.on("floor_button_pressed", function(floorNum) {
                floorButtonPressed(elevator, floorNum, "default", "NoLog");
            });
            elevator.on("passing_floor", function(passFloorNum, direction) {
                passingFloor(elevator, passFloorNum, 6, direction, "ShowLog");    
            });
        ...
        _.each(floors, function(floor) {
            floor.on("down_button_pressed", function() {
                ButtonPressedOnFloor( floor.floorNum(), "down_button_pressed", 4, 9999, false, "ShowLog" );
            });             
            floor.on("up_button_pressed", function() {
                ButtonPressedOnFloor( floor.floorNum(), "up_button_pressed", 4, 9999, false, "ShowLog" );
            });             
        });     
    },
