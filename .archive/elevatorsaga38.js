
{
    init: function(elevators, floors) {
        ...
        waitingInsideElevators = Array2D(elevators.length,floors.length,-1); // waitingInsideElevators[2][11] == 20;  A harmadik liftben 11. emeletre 20mp-e várnak EHELYETT elevators[2].waitingInside[11]
        ...
        listPassengersWeightInElevator = function(elevatorNumber, msg, logparam) {      
            ...
            for (var e = 0 ; logparam == "ShowEverything" && e < elevators.length ; e++) {
                ...
                for (var c = (counter)-20 ; (counter) > 25 && c <= (counter) ; c++ ) {                  
                    if (passengersWeightInElevators[c][e]>0) {                        
                        console.log("PassengersInElevator[" + c + "][" + e + "]=" + passengersWeightInElevators[c][e].toFixed(2) );  
                        if (prevEntries++ > 2)
                            break;                            
                    }                    
                }
            ...
            for (var c = counter ; c>0 ; c-- ) {                      
                if(currLoad==0 && passengersWeightInElevators[c][elevatorNumber]>0){
                    currLoad = passengersWeightInElevators[c][elevatorNumber];
                    continue;
                }
                if (prevLoad==0 && passengersWeightInElevators[c][elevatorNumber]>0 && currLoad!=0) {
                    prevLoad = passengersWeightInElevators[c][elevatorNumber];                
                    break;
                }
            }
        ...
        floorButtonPressed = function (elevator, FloorBtnPressed, behavior, logparam) {             
            ...
            passengersWeightInElevators[counter][elevator.index] = elevator.loadFactor();
